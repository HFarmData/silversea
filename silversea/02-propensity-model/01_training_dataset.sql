-- =============================================================================
-- SILVERSEA PROPENSITY — TRAINING DATASET v3   [RICOSTRUITO dalla specifica]
-- =============================================================================
-- ATTENZIONE: questo builder e' RICOSTRUITO dalla specifica v3 (il file originale
-- era un documento separato). Struttura CTE e feature sono documentate; i pattern
-- URL/param Silversea sono segnalati con "-- << CONFERMARE": allinearli ai builder
-- originali prima dell'uso in produzione.
--
-- Anchor: settimanali Set 2024 - Mar 2025. Observation window: 30 gg precedenti.
-- Prediction window: 14 gg successivi. Target: generate_lead_WBOF_01.
-- _TABLE_SUFFIX deve coprire anchor + 14 gg (fino ad aprile 2025).
-- Output: una riga per (user_pseudo_id, anchor_date) con visits_30d > 0.
-- =============================================================================

CREATE OR REPLACE TABLE `dm-2021-hdm-01.Silversea_Playground.propensity_training_v3` AS

WITH

-- 1) Date di riferimento (anchor) settimanali
anchor_dates AS (
  SELECT anchor_date
  FROM UNNEST(GENERATE_DATE_ARRAY(
    DATE 'AAAA-MM-GG',   -- << anchor start (originale 2024-09-01)
    DATE 'AAAA-MM-GG',   -- << anchor end   (originale 2025-03-15)
    INTERVAL 7 DAY
  )) AS anchor_date
),
...
events AS (
  ...
  WHERE _TABLE_SUFFIX BETWEEN '20240801' AND '20250414'   -- << copre anchor + 14gg
...
target_labels AS (
  ...
  FROM `silversea-293815.analytics_256550454.events_*` e
  CROSS JOIN anchor_dates a
  WHERE _TABLE_SUFFIX BETWEEN '20240801' AND '20250414'
